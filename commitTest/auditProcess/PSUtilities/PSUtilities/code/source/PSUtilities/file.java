...

{
        ...
        throws ServiceException
	{
		...
		try
		{
		  ...
		  FileOutputStream writer = new FileOutputStream(file.getCanonicalPath(), a); // append
		                                                                              // or
		                                                                              // not
		  writer.write(bytes);
		  writer.close();
	...
	public static final void getMimeType (IData pipeline)
        throws ServiceException
	{
		// --- <<IS-START(getMimeType)>> ---
		// @subtype unknown
		// @sigtype java 3.5
		// [i] field:0:required filename
		...
		if (idc.first("filename"))
		{
		  ...
		  mimeType = MimeTypes.getTypeFromName(filename);
		  if (mimeType == null)
		  {
		    mimeType = "text/html";
		  }
		...
		idc.insertAfter("mimeType", mimeType);
		idc.destroy();
		// --- <<IS-END>> ---
        ...
        throws ServiceException
	{
		...
		try
		{
		  ...
		  // Possible values of r: (1) r > 0, or (2) r < 0 when eof is reached; no r
		  // = 0.
	...
	// --- <<IS-START-SHARED>> ---
	private static final boolean checkPathValidity(String strPath, String strAction) throws Exception
	{
	  try
	  {
