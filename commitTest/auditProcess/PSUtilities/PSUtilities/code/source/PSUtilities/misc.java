...

{
    ...
    public static final Values deepConvert (Values in)
    {
		...
		/**
		 * This method will deep convert a Hashtable to a Values object. That is, it
		 * will be designed to convert all Hashtable objects within a hashtable,
		 * nested or not, to a values object.
		 * 
		 */
        ...
        throws ServiceException
	{
		...
		/**
		 * Service is designed to return the current service name. Output:
		 * folderPath - the folder path to the service serviceName - the service
		 * name fullName - the folder path + ":" + service successFlag - true or
		 * false errorMessage - error detail. This is set to "None" if no error
		 * occurs.
		 * 
		 * NOTE 1: Because this service relies on a method invocation that retrieves
		 * the NSService object relating to the calling service, it will *NOT* work
		 * as desired if run independently. Instead, it will return information on
		 * the current service (this one) and set the "successFlag" and
		 * "errorMessage" values to indicate an error. NOTE 2: This service uses
		 * non-public APIs within the webMethods Integration Server. These may
		 * change in future releases of the product *without* notice. These method
		 * invocations are marked as such.
		 * 
		 */
		...
		// Service Body
		// This publicly documented method gets the NSService object associated with
		// the calling service.
		callingService = Service.getCallingService();
		
		/**
		 * If the callingService is not available (meaning that the service was run
		 * directly, set the "callingService" to hold the values for the current
		 * service (this one). Additionally, set the "successFlag" and
		 * "errorMessage" fields to indicate a problem.
		 */
		...
		/**
		 * Non-public API. However, this call is relatively safe, as "toString" is a
		 * standard method that is normally overriden from the root
		 * java.lang.Object.
		 */
		fullName = callingService.toString();
		
		/**
		 * Use StringTokenizer, from the java.util package, to extract the
		 * folderPath and serviceName values.
		 */
		try
		{
	...
	public static final void spawnThread2 (IData pipeline)
        throws ServiceException
	{
		// --- <<IS-START(spawnThread2)>> ---
		// @sigtype java 3.5
		// [i] field:0:required serviceName
		// [o] object:0:required threadHandle
		...
		try
		{
		  ...
		  // Invoke service
		  ServiceThread threadHandle = Service.doThreadInvoke(ifc, service, IDataUtil.deepClone(pipeline));
		  idcPipeline.insertAfter("threadHandle", threadHandle);
		}
		catch (Exception e)
		{
